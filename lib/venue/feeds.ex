defmodule Venue.Feeds do
  ...
  def list_invite(conn) do
   ...
   reverse_following = Enum.map(c_user.reverse_relationships, fn member -> member.id end)
   query = from(p in Feed, where: p.type == 1 and p.user_id != ^current_user.id and p.relation_id in ^reverse_following, order_by: [desc: :updated_at], preload: [:user, :relation], limit: 5)
