defmodule Venue.Accounts do
  ...
  def list_users(conn) do
    if conn.assigns[:current_user] do
      c_user = get_user!(conn.assigns.current_user.id)
        ...
        from(p in User,
          where:
            p.id != ^current_user.id and
              st_distance_in_meters(p.geom, ^c_user.geom) < ^c_user.distance * 1000,
          order_by: [desc: :updated_at],
          preload: :reverse_relationships,
          limit: 10
        )
  ...
  def list_following(conn) do
    if conn.assigns[:current_user] do
      ...
      query =
        from(p in User,
          where: p.id != ^current_user.id and p.id in ^following,
          order_by: [desc: :updated_at]
        )
  ...
  def list_friends(conn) do
    if conn.assigns[:current_user] do
      ...
      query =
        from(p in User,
          where: p.id != ^current_user.id and p.id in ^reverse and p.id in ^following,
          order_by: [desc: :updated_at],
          preload: :reverse_relationships
        )
